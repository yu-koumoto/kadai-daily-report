...
public class EmployeesUpdateServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // 重複チェックを行う指定をする
        Boolean codeDuplicateCheckFlag = true;//Flagの初期値をtrueとする
        ...
        if(password == null || password.equals("")){//equals("")空の値入っている、nullガチで空
        ...
        if(errors.size() > 0){
            ...
            em.getTransaction().begin();//始める
            em.getTransaction().commit();//確定させる？
            em.close();
            request.getSession().setAttribute("flush","更新しました" );
            request.getSession().removeAttribute("employee_id");//removeで消してる

            response.sendRedirect(request.getContextPath() + "/employees/index");//indexに帰る命令をだす。
